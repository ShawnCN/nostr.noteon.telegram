        ...
        switch (forward_info['@type']) {
            case 'messageForwardedFromUser': {
                if (forward_info.sender_user_id > 0) {
                    ...
                    if (user) {
                        return getUserFullName(forward_info.sender_user_id, null);
                    }
                }
                break;
            }
...
function getTitle(message, t = k => k) {
    ...
    if (user_id) {
        ...
        if (user) {
            return getUserFullName(user_id, null);
        }
    }
...
function getFormattedText(formattedText, t = k => k, options = { } as any) {
...
function getForwardTitle(forwardInfo, t = key => key) {
    ...
    switch (origin['@type']) {
        case 'messageForwardOriginUser': {
            ...
            return getUserFullName(sender_user_id, null);
        }
        case 'messageForwardOriginHiddenUser': {
        ...
        case 'messageForwardOriginChannel': {
            ...
            return getChatTitle(chat_id, false) + (author_signature ? ` (${author_signature})` : '');
        }
    }
...
export function useAudioPlaybackRate(source) {
    ...
    let audio = null as unknown as any;
    switch (source['@type']) {
        case 'message': {
                ...
                case 'textEntityTypeMentionName': {
                    try {
                        ...
                        if (user) {
                            ...
                            node.title = getUserFullName(user_id, null);
                            // node.target = '_blank';

...
class Message extends Component<IPropsMessage, IStateMessage> {
  static defaultProps = {
    sendingState: null,
    showTitle: false,
    showTail: false,
    ...
    updateMessageContent: any;
    updateMessageEdited: any;
    mouseDown: any;
    mouseOut: boolean;
  constructor(props) {
    super(props);
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    const { contextMenu, selected, highlighted, shook, emojiMatches } =
      this.state;
  ...
  componentDidMount() {
    ...
    MessageStore.on(
      'clientUpdateMessageShake',
      this.onClientUpdateMessageShake
    );
    MessageStore.on(
  ...
  handleOpenContextMenu = async (event) => {
    ...
    if (contextMenu) {
      ...
      const copyLink =
        event.target && event.target.tagName === 'A' && event.target.href
          ? event.target.href
          : null;
  ...
  render() {
    ...
    const message:TMsg = MessageStore.get(chatId, messageId);
    ...
    const suppressTitle =
      (isPrivateChat(chatId) && !(isMeChat(chatId) && !isOutgoing)) ||
      (isGroupChat(chatId) && isOutgoing);
    ...
    let tile:ReactElement<any, any> | null = null;
    if (showTail) {
        ...
        switch (forward_info.origin['@type']) {
          case 'messageForwardOriginHiddenUser': {
            tile = (
              <UserTile
                small
                firstName={forward_info.origin.sender_name}
          ...
          case 'messageForwardOriginUser': {
            tile = (
              <UserTile
                small
                userId={forward_info.origin.sender_user_id}
                onSelect={this.handleSelectUser}
              />
            );
            break;
          ...
          case 'messageForwardOriginChannel': {
            tile = (
              <ChatTile
                small
                chatId={forward_info.origin.chat_id}
    ...
    const style:any = getMessageStyle(chatId, messageId);
    const withBubble =
    ...
    const tailRounded =
      (!hasCaption &&
        (content['@type'] === 'messageAnimation' ||
          ...
          (content['@type'] === 'messageInvoice' && content.photo))) ||
      (reply_markup && reply_markup['@type'] === 'replyMarkupInlineKeyboard');
          ...
          <div className='message-body'>
            ...
            <div className={classNames('message-wrapper', { shook: shook })}>
              ...
              <div>
                ...
                >
                  {withBubble &&
                    ((showTitle && !suppressTitle) || showForward) && (
...
const message = withTranslation(['translation', 'local'], { withRef: true })(
  Message
);

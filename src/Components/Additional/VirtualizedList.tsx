...
interface IPropsVirtualizedList {
  renderItem: ({ index, style }: { index: number; style: any }) => void;
...
interface IStateVirtualizedList {
  scrollTop: number;
  renderIds: Map<any, any>;
  renderIdsList: any[];
  viewportHeight: number;
}
export class VirtualizedList extends React.Component<
  IPropsVirtualizedList,
  IStateVirtualizedList
> {
  static defaultProps = {
    source: [],
    rowHeight: 72,
  ...
  componentDidMount() {
    ...
    const { current }: { current: any } = this.listRef;
    if (!current) return;
  ...
  getRenderIds(source, viewportHeight: number, scrollTop: number) {
    const renderIds = new Map();
    const renderIdsList: number[] = [];
  ...
  isVisibleItem = (index:number, viewportHeight:number, scrollTop:number) => {

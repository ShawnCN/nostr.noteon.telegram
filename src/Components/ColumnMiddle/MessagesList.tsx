...
import {
  ALBUM_MESSAGES_LIMIT,
  MESSAGE_SLICE_LIMIT,
  MESSAGE_SPLIT_MAX_TIME_S,
  SCROLL_PRECISION,
} from '../../Constants';
...
class MessagesList extends React.Component<
  ...
  IStateMessagesList
> {
  lastRequests: Map<any, any>;
  listRef: React.RefObject<unknown>;
  messages: React.ReactElement<any, any>[] | null;
  itemsMap: any;
  sessionId: {
    date: Date;
    loading: boolean;
    completed: boolean;
    loadMigratedHistory: boolean;
  };
  itemsRef: React.RefObject<unknown>;
  defferedActions: any[];
  scrollBehaviorNone: boolean;
  inViewMap: Map<any, any>;
  // @ts-ignore
  scrollDownButtonRef: LegacyRef<ScrollDownButton> | undefined;
  snapshot: { scrollTop: any; scrollHeight: any; offsetHeight: any };
  prevOffsetHeight: any;
  prevScrollTop: any;
  scrollBottomAfterEditing: { chatId: number; messageId: number } | null;
  cancelUpdatePinnedMessage: any;
  prevHistory: any[];
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      replyHistory: [],
    } as unknown as IStateMessagesList;
  ...
  hasLastMessage() {
    ...
    return (
      history.length > 0 && history[history.length - 1].id >= last_message.id
    );
  }

  static getDerivedStateFromProps(props, state) {
    if (
      props.chatId !== state.prevChatId ||
      ...
      return {
        ...
        selectionActive: false,
        separatorMessageId:
          props.chatId !== state.prevChatId ? 0 : state.separatorMessageId,
        scrollDownVisible:
          props.chatId === state.prevChatId &&
          (state.scrollDownVisible || state.replyHistory.length > 0),
        replyHistory:
          props.chatId !== state.prevChatId ? [] : state.replyHistory,
      };
    }
  ...
  componentDidMount() {
    ...
    MessageStore.on(
      ...
      this.onClientUpdateSelection
    );
    MessageStore.on('clientUpdateOpenReply', this.onClientUpdateOpenReply);
    MessageStore.on(
      'clientUpdateStartMessageEditing',
      this.onClientUpdateStartMessageEditing
    );
    MessageStore.on(
      'clientUpdateStopMessageEditing',
      this.onClientUpdateStopMessageEditing
    );
    MessageStore.on(
      'clientUpdateTryEditMessage',
      this.onClientUpdateTryEditMessage
    );
    ...
    MessageStore.on(
      ...
      this.onUpdateMessageSendSucceeded
    );
    MessageStore.on(
  ...
  onClientUpdateStartMessageEditing = (update) => {
    ...
    const { current: list }: { current: any } = this.listRef;
    if (list.offsetHeight + list.scrollTop < list.scrollHeight) {
      return;
    }

    if (!history.some((x) => x.chat_id === chatId && x.id === messageId))
      return;
  ...
  onUpdateMessageIsPinned = (update) => {
    ...
    if (is_pinned) {
      ...
      let scrollBehavior =
        message.is_outgoing && !isServiceMessage(message)
          ? ScrollBehaviorEnum.SCROLL_TO_BOTTOM
          : ScrollBehaviorEnum.NONE;
  ...
  onClientUpdateFocusWindow = (update) => {
    ...
    if (focused) {
      this.defferedActions.forEach((x: any) => x());
      this.defferedActions = [];
    }
  };
  ...
  onClientUpdateOpenReply = (update) => {
    ...
    const lastItem =
      replyHistory.length > 0 ? replyHistory[replyHistory.length - 1] : null;
    if (
      lastItem &&
      lastItem.chatId === chatId &&
      lastItem.messageId === messageId
    ) {
      return;
    }
  ...
  async handleSelectChat(
    ...
    const scrollPosition = null as unknown as any; //ChatStore.getScrollPosition(chatId);
    ...
    if (chat) {
      ...
      const offset =
        unread || messageId || scrollPosition ? -1 - MESSAGE_SLICE_LIMIT : 0;
      const limit =
        unread || messageId || scrollPosition
      ...
      if (chatId !== previousChatId) {
        ...
        separatorMessageId =
          separatorMessageId === Number.MAX_VALUE ? 0 : separatorMessageId;
      }

      let scrollBehavior: any = ScrollBehaviorEnum.SCROLL_TO_BOTTOM;
      if (messageId) {
    ...
    if (previousChatId !== chatId && !this.props.filter) {
      if (previousChat) {
        ...
        if (message) {
          ...
          ChatStore.setScrollPosition(previousChatId, {
            chatId,
            messageId,
            offset: scrollMessage.offset,
          });
        }
      }
    }
  ...
  getMessageSticker(message: any): [any, any] {
    throw new Error('Method not implemented.');
  }
  getMessagePhoto(message: any): [any, any] {
    throw new Error('Method not implemented.');
  }
  ...
  onLoadNext = async () => {
    ...
    const limit =
      history.length < MESSAGE_SLICE_LIMIT
        ? MESSAGE_SLICE_LIMIT * 2
        : MESSAGE_SLICE_LIMIT;
    ...
    if (this.lastRequests.has(lastRequestKey)) {
      ...
      sessionId.loading = true;
      result = await this.getRequest(chatId, fromMessageId, 0, limit).finally(
        () => {
          sessionId.loading = false;
        }
      );
  ...
  onLoadMigratedHistory = async () => {
    ...
    const fromMessageId =
      history.length > 0 && history[0].chat_id === basicGroupChat.id
        ? history[0].id
        : 0;
    const limit =
      fromMessageId === 0 ? MESSAGE_SLICE_LIMIT * 2 : MESSAGE_SLICE_LIMIT;
  ...
  onLoadPrevious = async () => {
    ...
    const fromMessageId =
      history && history.length > 0 ? history[history.length - 1].id : 0;
    const limit =
      history.length < MESSAGE_SLICE_LIMIT
        ? MESSAGE_SLICE_LIMIT * 2
        : MESSAGE_SLICE_LIMIT;
            ...
            this.setState(
              {
                history: currentHistory.slice(
                  currentHistory.length - MESSAGE_SLICE_LIMIT * 3
                ),
              },
              () => {
                // @ts-ignore
  ...
  updatePinnedMessage = (scrollToNext) => {
    ...
    if (
      id &&
      (!currentPinned ||
        (currentPinned.chatId === chatId && currentPinned.id !== id))
    ) {
      if (
        scrollToNext &&
        clickedPinned &&
        clickedPinned.chatId === chatId &&
        clickedPinned.id < id
      ) {
      ...
      TdLibController.clientUpdate({
        '@type': 'clientUpdateCurrentPinnedMessage',
        chatId,
        messageId: id,
      });
    } else {
  ...
  scrollToPosition = (position) => {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      const node: any = ReactDOM.findDOMNode(itemComponent);
      if (node) {
  ...
  scrollToMessage = () => {
    ...
    for (let i = 0; i < history.length; i++) {
      ...
      if (node) {
        if (
          itemComponent.props.messageId === messageId ||
          (itemComponent.props.messageIds &&
            itemComponent.props.messageIds.indexOf(messageId) !== -1)
        ) {
  ...
  scrollToStart = async () => {
    ...
    const hasLastMessage = history.some(
      (x) =>
        x.chat_id === chatId &&
        chat &&
        chat.last_message &&
        chat.last_message.id === x.id
    );
    if (hasLastMessage) {
      this.scrollToBottom();
    } else {
  ...
  showMessageTitle(message, prevMessage, isFirst, isFirstUnread) {
    ...
    const { chat_id, date, is_outgoing, sender_id, content, forward_info } =
      message;
    ...
    return (
      ...
      (isServiceMessage(prevMessage) ||
        ...
        (isMeChat(chat_id) &&
          !forwardInfoEquals(forward_info, prevMessage.forward_info)) ||
    ...
    if (clearHistory) {
      ...
      for (let i = 0; i < history.length; i++) {
        ...
        if (
          ...
          for (
            let j = i + 1;
            j < i + ALBUM_MESSAGES_LIMIT && j < history.length;
            j++
          ) {
          ...
          if (album.length > 1) {
            ...
            const nextMessage =
              i + album.length < history.length
                ? history[i + album.length]
                : null;

            const showDate = this.showMessageDate(x, prevMessage, i === 0);

            const isFirstUnread = separatorMessageId === x.id;
            const isNextFirstUnread = nextMessage
              ? separatorMessageId === nextMessage.id
              : false;
            const showTitle = this.showMessageTitle(
              x,
              prevMessage,
              i === 0,
              isFirstUnread
            );
            const nextShowTitle = this.showMessageTitle(
              nextMessage,
              x,
              false,
              isNextFirstUnread
            ...
            const showTail =
              ...
              (isMeChat(x.chat_id) &&
                !forwardInfoEquals(x.forward_info, nextMessage.forward_info)) ||
            ...
            this.messages.push(
              <Album
                ...
                ref={(el) =>
                  album.forEach((x, index) => {
                    this.itemsMap.set(i + index, el);
                  })
                }
                chatId={chat_id}
        ...
        if (!albumAdded) {
          ...
          let m: null | ReactElement<any, any> = null;
          if (isServiceMessage(x)) {
            ...
            const isFirstUnread = separatorMessageId === x.id;
            const isNextFirstUnread =
              nextMessage && separatorMessageId === nextMessage.id;
            const showTitle = this.showMessageTitle(
              x,
              prevMessage,
              i === 0,

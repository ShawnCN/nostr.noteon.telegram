...
import { achats, aresult } from '../../data';
...
interface IStateDialogListItem {
  hiddenChats: Map<any, any>;
}

class DialogListItem extends React.Component<
  IPropsDialogListItem,
  IStateDialogListItem
> {
...
> {
  ...
  onUpdateChatOrder = (update:{ chat_id:number }) => {
    ...
    if (order === '0') {
      ...
      if (currentIndex === -1) {
        if (loading) {
          console.error(`[vl] skip ${update['@type']}`, {
            id: chat_id,
            title: ChatStore.get(chat_id)?.title,
            chat: ChatStore.get(chat_id),
          });
  ...
  reorderChats(chatIds, newChatIds = [] as number[], callback) {
    const { chatList } = this.state;
  ...
  async onLoadNext(replace = false, limit = CHAT_SLICE_LIMIT) {
    ...
    let offsetChatId = 0;
    let offsetChat = undefined as TChat | undefined;
    ...
    params.loading = true;
    let result: { chat_ids: number[] } = await TdLibController.send({
      '@type': 'getChats',
      chat_list: chatList,
      limit: 300,
    }).finally(() => {
    ...
    if (type === 'chatListMain')
      console.log('[vl] GETCHATS stop', replace, type, result);
  ...
  appendChats(chatIds:number[], callback) {
    if (chatIds.length === 0) {
      if (callback) callback();
      return;
    }
  ...
  renderItem = ({ index, style }:{index:number,style:TStyle}, source: number[], stub = false) => {
    const { chatList } = this.state;
  ...
  render() {
    ...
    let { chats, chatList } = this.state;
    chats = [10, 11];

    this.source = [] as number[];
    this.stub = false;
    if (chats) {
      let lastPinnedId = 0;
      chats.forEach((x) => {
